...
def listar_vizinhos(grafo, vertice):
    ...
    if vertice in grafo:
        print(f"Vizinhos de {vertice}: {sorted(lista)}")
    ...
    while fila:  
        ...
        if visitado not in visitados:
            ...
            if vizinhos_do_visitado:  
                for vizinho in sorted(vizinhos_do_visitado):
...
def BFS_menor_caminho(grafo, origem, destino):
    ...
    while fila:
        ...
        if vizinhos_do_vertice:
            for vizinho in sorted(vizinhos_do_vertice):
...
def DFS_Ciclos(grafo, vertice): 
    ...
    while pilha:  
        ...
        if vertice_atual not in visitados:
            ...
            vizinhos_do_visitado = vizinhos(grafo, vertice_atual)
            ...
            if vizinhos_do_visitado:  
                for vizinho in sorted(vizinhos_do_visitado):
                    if vizinho not in visitados:
                        ...
                        if vizinho != pai:
                            print(f"Ciclo detectado. Aresta de volta: {vertice_atual} -> {vizinho}")
                            return True
...
def main():
    def menu(grafo, eh_nao_direcionado):
        while True:
            ...
            print("12 - Busca em Profundidade para Detecção de Ciclos")
            print("0 - Sair")
            ...
            if escolha == "1":
                ...
                vertice_inicial = input("Digite o vértice inicial: ").strip()
                ordem_pilha = DFS_Padrao(grafo, vertice_inicial) 
        
                if ordem_pilha:
                    print(f"Ordem DFS: {ordem_pilha}") 
            
            elif escolha == "12":
                vertice_inicial = input("Digite o vértice inicial: ").strip()
                tem_ciclo = DFS_Ciclos(grafo, vertice_inicial) 
        
                if tem_ciclo:
                    print("Resultado: O grafo possui ciclos.") 
                else:
                    print("Resultado: Nenhum ciclo detectado a partir deste vértice.")
